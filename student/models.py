...
class ClassModel(BaseModel):
    #请根据题目说明自行补全完整的Model
    name: str = models.CharField(verbose_name="班级名称",
                                 max_length=64,
                                 help_text="班级名称")
...
class StudentModel(BaseModel):
    ...
    user: User = models.OneToOneField(settings.AUTH_USER_MODEL,
                                      verbose_name="用户 与学生1:1",
                                      on_delete=models.CASCADE)  #联级删除
    sex: SEX = models.SmallIntegerField(verbose_name="性别",
                                        choices=[(r.name, r.value)
                                                 for r in SEX])
    cs: ClassModel = models.ForeignKey("ClassModel",
                                       on_delete=models.CASCADE)  #联级删除
